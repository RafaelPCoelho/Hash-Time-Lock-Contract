...
async function main() {
    ...
    if (i=0 || i/2){
        ...
        htlcSepolia = await HTLC.connect(arbitrumWallet).deploy(sepoliaWallet.address, tokenSepolia.getAddress(), tokenIdSepolia);
        htlcArbitrum = await HTLC.connect(sepoliaWallet).deploy(arbitrumWallet.address, tokenArbitrum.getAddress(), tokenIdArbitrum);
    }
